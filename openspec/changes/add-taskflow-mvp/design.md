...
### 8. 입력 한계: 메시지 1000자, 태스크 제목 200자 (양쪽 서버 강제)

**선택 이유:** Pydantic `max_length`로 서버에서 일관 강제. 메시지/태스크 둘 다 DB 폭발과 거대 페이로드 차단. FE는 표시 단계에서 추가로 ellipsis 처리.

### 9. 에러 응답 포맷: `{code, msg}` 일관

**선택 이유:** AI 구현 시 추측 여지 차단. 모든 4xx/5xx 응답은 동일 포맷.

### 10. 초대코드 형식: `XXXX-XXXX` (영숫자 대문자+숫자, 4-4 하이픈)

**대안:** 짧은 6자, 긴 UUID 일부, 하이픈 없는 8자 연속.
**선택 이유:** 프로그램정의 PDF의 ACME Examples(`ABCD-1234`)와 일치. 하이픈으로 가독성↑·복붙·구두 전달 모두 용이. 36^8 ≈ 2.8조 조합으로 MVP 규모 충분.
...
- **[초대코드 추측 공격]** → `XXXX-XXXX` 영숫자 대문자+숫자(36^8 ≈ 2.8조) + UNIQUE 제약. 무차별 시도는 MVP에서 별도 차단(rate limit) 없음.
...
초기 MVP는 아래 기본값을 채택. 후속 이터레이션에서 재검토 가능:

- **메시지 삭제 권한 확장**: 현재 본인만 → 향후 팀 admin도 허용 검토
- **태스크 삭제 권한 축소**: 현재 멤버 누구나 → 향후 작성자/admin 한정 검토
